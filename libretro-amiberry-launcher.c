...
void retro_get_system_info(struct retro_system_info *info)
{
   ...
   info->library_name     = "Retropie Amiberry Launcher";
   info->library_version  = "1.0.4";
   info->need_fullpath    = true;
   info->valid_extensions = "lha|zip|adf";
}
...
{
   // Construct the command to run Amiberry.
   char command[512] = "cd /opt/retropie/emulators/amiberry; ./amiberry -autoload=";

   // Check if there is content to load.
   if (info != NULL && info->path != NULL && info->path[0] != '\0') {
      sprintf(command, "%s\"%s\" -G", command, info->path);
   }

   //log_cb(RETRO_LOG_INFO, "command: %s , path:  %s.\n", command, info->path);
